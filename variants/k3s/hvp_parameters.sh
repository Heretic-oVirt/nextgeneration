...
# Note: to set cluster join token add hvp_jointoken=XXX where XXX is the token (automatically generated only if not specified for server node 0 otherwise should be copied from /var/lib/rancher/k3s/server/token)
...
default_removepkgs=""
default_replacepkgs=""
default_addpkgs="tmux https://github.com/k3s-io/k3s-selinux/releases/download/v1.5.stable.1/k3s-selinux-1.5-1.coreos.noarch.rpm"
default_removekargs=""
...
function post_install_hook_custom_actions() {
	...
	case "${given_nodetype}" in
		server)
			echo "Setting server role for K3s node" 1>&2
			k3s_options="server --flannel-backend=host-gw --disable servicelb --disable traefik --selinux"
			;;
		*)
			# In any other case use agent as default type
			echo "Setting agent role for K3s node" 1>&2
			given_nodetype="agent"
	...
	case "${given_nodetype}" in
		server)
			...
			if [ "${given_nodeid}" -eq 0 ]; then
				...
				default_enablesvcs="${default_enablesvcs} configure-k3s.service"
			fi
			k3s_service="k3s"
			;;
		agent)
	...
	cat <<- EOF > "/mnt/${ostree_path}/etc/NetworkManager/conf.d/k3s.conf"
	[keyfile]
	unmanaged-devices=interface-name:cali*;interface-name:flannel*
	EOF
	chown root:root "/mnt/${ostree_path}/etc/NetworkManager/conf.d/k3s.conf"
	chmod 644 "/mnt/${ostree_path}/etc/NetworkManager/conf.d/k3s.conf"
	
	# Add configuration file required by install-k3s.service and configure-k3s.service
	...
	# Define K3s configuration parameters
	...
	ExecStartPost=/usr/sbin/restorecon -Rv /usr/local/bin
	# Note: K3s creates some needed dirs/files only on first execution
	ExecStartPost=/usr/bin/systemctl --now enable ${k3s_service}.service
	# Note: kubectl link does not get automatically created if already provided by a builtin package
	ExecStartPost=/usr/bin/ln -sf k3s /usr/local/bin/kubectl
	...
	ExecStartPost=/usr/bin/ln -sf k3s /usr/local/bin/crictl
	...
	ExecStartPost=/usr/bin/ln -sf k3s /usr/local/bin/ctr
	...
	ExecStartPost=/usr/bin/bash -c '/usr/bin/echo "export KUBECONFIG=/etc/rancher/k3s/k3s.yaml" > /etc/profile.d/k3s.sh'
	ExecStartPost=/usr/bin/bash -c '/usr/bin/echo "setenv KUBECONFIG /etc/rancher/k3s/k3s.yaml" > /etc/profile.d/k3s.csh'
	...
	if [ "${given_nodetype}" = "server" -a "${given_nodeid}" -eq 0 ]; then
		...
		cat <<- EOF > "/mnt/${ostree_path}/etc/rancher/k3s/metallb-config.yaml"
		...
		spec:
		  ...
		  - ens192
		EOF
		chown root:root "/mnt/${ostree_path}/etc/rancher/k3s/metallb-config.yaml"
		chmod 644 "/mnt/${ostree_path}/etc/rancher/k3s/metallb-config.yaml"
	
		# Add configuration file for OpenEBS
		mkdir -p "/mnt/${ostree_path}/etc/rancher/k3s"
		cat <<- EOF > "/mnt/${ostree_path}/etc/rancher/k3s/openebs-config.yaml"
		...
		EOF
		chown root:root "/mnt/${ostree_path}/etc/rancher/k3s/openebs-config.yaml"
		chmod 644 "/mnt/${ostree_path}/etc/rancher/k3s/openebs-config.yaml"
		...
		mkdir -p "/mnt/${ostree_path}/etc/rancher/k3s"
		cat <<- EOF > "/mnt/${ostree_path}/etc/rancher/k3s/rancher-values.yaml"
		...
		replicas: 1
		EOF
		chown root:root "/mnt/${ostree_path}/etc/rancher/k3s/rancher-values.yaml"
		chmod 644 "/mnt/${ostree_path}/etc/rancher/k3s/rancher-values.yaml"
	
		# Add specialized configuration unit
		cat <<- EOF > "/mnt/${ostree_path}/etc/systemd/system/configure-k3s.service"
		[Unit]
		Description=Configure K3s and further required components
		# Run after required RPM dependencies have been installed and K3s has been installed
		...
		ConditionPathExists=/etc/sysconfig/configure-k3s.allowed
		# Do not execute anymore if it was already installed
		ConditionPathExists=!/var/lib/%N.stamp
		...
		# TODO: add optional MultusCNI and KubeVirt deployments
		ExecStart=/usr/local/bin/helm repo add metallb https://metallb.github.io/metallb
		ExecStartPost=/usr/local/bin/helm repo add ingress-nginx https://kubernetes.github.io/ingress-nginx
		...
		ExecStartPost=/usr/local/bin/kubectl -n metallb-system apply -f /etc/rancher/k3s/metallb-config.yaml
		ExecStartPost=/usr/local/bin/helm install nginx ingress-nginx/ingress-nginx --create-namespace --namespace nginx-system --version \${INSTALL_NGINX_VERSION}
		ExecStartPost=/usr/local/bin/kubectl -n nginx-system rollout status deploy/nginx-ingress-nginx-controller
		...
		ExecStartPost=/usr/bin/bash -c '/usr/bin/test "x\${INSTALL_OPENEBSLOCAL_VG}" != "x" && /usr/local/bin/kubectl -n openebs apply -f /etc/rancher/k3s/openebs-config.yaml || true'
		# TODO: debug rancher ingress/TLS problems
		ExecStartPost=/usr/bin/bash -c '/usr/bin/test "x\${INSTALL_RANCHER_VERSION}" != "x" && /usr/local/bin/helm install rancher rancher-\${INSTALL_RANCHER_CHANNEL}/rancher --create-namespace --namespace cattle-system --version \${INSTALL_RANCHER_VERSION} --values /etc/rancher/k3s/rancher-values.yaml || true'
		...
		WantedBy=multi-user.target
		EOF
		chown root:root "/mnt/${ostree_path}/etc/systemd/system/configure-k3s.service"
		chmod 644 "/mnt/${ostree_path}/etc/systemd/system/configure-k3s.service"
	fi
}
