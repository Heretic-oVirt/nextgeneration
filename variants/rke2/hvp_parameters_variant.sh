...
# Note: to set loadbalanced services IP range add hvp_lbiprange=A.B.C.D-E.F.G.H where A.B.C.D is the first and E.F.G.H is the last IP address available

variant_type="RKE2"
variant_version="2024092301"
...
[ -z "${default_nodetype+x}" ] && default_nodetype="server"
[ -z "${default_nodeid+x}" ] && default_nodeid=0
...
function pre_install_hook_custom_actions() {
	...
	# Determine the loadbalanced service IP range
...
function post_install_hook_custom_actions() {
	...
	# TODO: use Kube-VIP instead of MetalLB
	...
	# Define ${variant_type} configuration parameters
	...
	if [ "${given_nodetype}" = "server" -a "${given_nodeid}" -eq 0 ]; then
		...
		[Unit]
		Description=Configure ${variant_type} and further required components
